...
- [API](#api)
  ...
  - [Поиск и каталог](#поиск-и-каталог)
  - [SKU endpoints](#sku-endpoints)
...
### 🛡️ Rate Limiting — защита от DDoS и злоупотреблений
...
### Шаг 1.5: Применить миграции (опционально для production)

> 💡 **О схеме БД**: `db/init.sql` создаёт полную рабочую схему с таблицами, индексами и функциями. Для быстрого старта разработки этого достаточно. Для production-окружения рекомендуется применить миграции (см. ниже).
...
### Шаг 4: Загрузить данные (опционально)
...
```json
{
  "status": "alive",
  "version": "0.4.0",
  "timestamp": "2025-10-30T19:03:38.813229Z",
  "uptime_seconds": 3600
}
```
...
{
  "status": "ready",
  "timestamp": "2025-10-30T18:17:37.635353Z",
  "version": "0.4.0",
  "checks": {
    "database": {
      "status": "up",
...
{
  "status": "not_ready",
  "timestamp": "2025-10-30T18:22:19.438239Z",
  "version": "0.4.0",
  "checks": {
    "database": {
      "status": "down",
...
- ✅ Версия БД (PostgreSQL 16+)
- ✅ Наличие 4 таблиц
- ✅ Наличие 3 критичных индексов
- ✅ Наличие CHECK constraint
...
### Использование скидки из ячейки
...
> ⚠️ **Важно**: Все API эндпоинты фильтруют и сортируют по финальной цене (`price_final_rub`).
...
### "Error tokenizing data" при загрузке CSV
...
### Rate limit headers не появляются
...
Начиная с версии 0.4.1, API использует structured JSON logging — все логи записываются в формате JSON, что позволяет легко анализировать их автоматическими системами мониторинга (Datadog, ELK, Splunk).

### Преимущества

- ✅ **Структурированность** — легко фильтровать и анализировать
- ✅ **Request Tracing** — каждый запрос имеет уникальный Request ID
- ✅ **Performance Metrics** — автоматическое измерение времени выполнения
- ✅ **Production-ready** — совместимость с системами мониторинга
...
```env
# Уровни логирования
LOG_LEVEL=DEBUG    # Детальные логи (для разработки)
LOG_LEVEL=INFO     # Стандартные логи (для production) — рекомендуется
...
Каждый ответ API содержит заголовок `X-Request-ID`, который совпадает с `request_id` в логах.
...
# Вывод: X-Request-ID: req_76a01810
```

**Применение:**
- 🐛 **Отладка** — найти все логи конкретного запроса
- 📞 **Поддержка** — попросить пользователя прислать Request ID
- 📊 **Аналитика** — проследить путь запроса через микросервисы
...
# Фильтрация по уровню (только ERROR)
docker compose logs api | Select-String '"level": "ERROR"'
```

### Интеграция с системами мониторинга

JSON формат позволяет легко интегрироваться с:
- **Datadog** — просто отправляйте логи через агент
- **ELK Stack** (Elasticsearch, Logstash, Kibana) — используйте Logstash для парсинга
- **Splunk** — импорт JSON логов из коробки
- **CloudWatch Logs** — для AWS deployment
- **Google Cloud Logging** — для GCP deployment

**Пример запроса в Datadog:**
...
**Проблема:** Видите plain text логи вместо JSON.

**Решение:**

```powershell
# Проверьте, что модули импортированы
...
**Проблема:** Заголовок `X-Request-ID` отсутствует.

**Решение:**

```powershell
# Проверьте, что middleware подключён
...
### 🔄 Спринт 4 — Testing & Quality (в разработке)
