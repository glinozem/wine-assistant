...
- [Быстрый старт](#быстрый-старт)
  ...
  - [1.5) Применить миграции (обязательно!)](#15-применить-миграции-обязательно)
...
⚠️ **Важно:** `db/init.sql` создаёт только минимальную схему. Для полной функциональности **обязательно примените миграции** (см. шаг 1.5).

---

### 1.5) Применить миграции (обязательно!)
...
**Миграции добавят:**
- Колонки `price_list_rub`, `price_final_rub` в `products`
- Таблицу истории цен `product_prices` (+ guardrails: без перекрытий, уникальный индекс)
- Таблицу истории остатков `inventory_history` и индексы
- Актуальные поля в `inventory` (`stock_total`, `reserved`, `stock_free`)
- Индексы и расширения (`pg_trgm`, `pgvector`, `btree_gist`)

⚠️ **Без миграций API не запустится!** Вы получите ошибку `column "price_final_rub" does not exist`.
...
- [ ] Миграции применены (`.\scripts\migrate.ps1`)
...
**Важно:** `db/init.sql` — минимальная схема для инициализации контейнера. Для полноценной работы **обязательно выполняйте миграции**.
