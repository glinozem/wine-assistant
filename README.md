...
💡 **О схеме БД:** `db/init.sql` создаёт полную рабочую схему с таблицами, индексами и функциями. Для быстрого старта разработки этого достаточно. Для production-окружения **рекомендуется дополнительно применить миграции** (см. шаг 1.5), которые добавляют guardrails и диагностику.
...
**Миграции добавляют:**
- 🔒 **Guardrails:** CHECK constraint на неотрицательность цен, нормализация интервалов в истории цен
- 📊 **Диагностика:** Запросы для проверки целостности данных (перекрытия интервалов, несогласованные остатки)
- ⚡ **Оптимизации:** Дополнительные индексы для production окружения
- 🔧 **Совместимость:** Миграция старых структур БД (колонки `qty`, `in_stock` → `stock_total`, `reserved`, `stock_free`)
...
- ✅ Базовые constraints (PRIMARY KEY, FOREIGN KEY)

**Миграции добавляют:**
- 🔒 **CHECK constraint** `chk_product_prices_nonneg` (цены неотрицательные)
- 🔍 **Диагностические запросы** для контроля качества данных (перекрытия интервалов, несогласованные остатки)
- 🔧 **Нормализация данных** (effective_to в product_prices, миграция старых колонок inventory)
- 🚀 **Production-специфичные оптимизации**

**Вывод:**
- **Dev-окружение:** Можно работать сразу после `docker compose up -d`
- **Production:** Рекомендуется выполнить миграции для дополнительных guardrails и диагностики
- **Обновление со старой версии:** Миграции обязательны (добавят недостающие колонки и обновят функции)
...
1. **2025-10-14-prices-and-search.sql** — добавление двух цен и истории
   - Добавляет колонки `price_list_rub`, `price_final_rub` в `products`
   - Создаёт таблицы `product_prices`, `inventory_history`
   - Создаёт функции `upsert_price()`, `upsert_inventory()`

2. **2025-10-14-price-check.sql** — CHECK constraint
   - Добавляет `chk_product_prices_nonneg` (цены неотрицательные)

3. **2025-10-14-price-history-guardrails.sql** — guardrails и нормализация
   - Уникальный индекс `ux_product_prices_code_from`
   - Нормализация `effective_to` (закрывает интервалы корректно)
   - ⚠️ **Примечание:** EXCLUDE constraint на перекрытия интервалов **не создаётся** автоматически из-за проблем с IMMUTABLE функциями. Целостность обеспечивается функцией `upsert_price()` и уникальным индексом.

4. **2025-10-14-diagnostics.sql** — диагностика
   - Запросы для проверки целостности данных

5. **2025-10-17-inventory-columns-and-asof.sql** — нормализация inventory
   - Добавляет `stock_total`, `reserved`, `stock_free`, `asof_date`
   - Мигрирует данные из старых колонок `qty`, `in_stock`
   - Обновляет функцию `upsert_inventory()`

**Примечание:** `db/init.sql` содержит полную актуальную схему (синхронизирован с миграциями в Спринте 1). Миграции нужны только для **обновления существующих БД** или для дополнительных guardrails в production.
...
**Решение:**
```powershell
# Вариант 1: Пересоздать БД (быстро, для dev-окружения)
docker compose down -v
docker compose up -d
# → db/init.sql создаст актуальную схему автоматически

# Вариант 2: Применить миграции (для production с данными)
powershell -ExecutionPolicy Bypass -File .\scripts\migrate.ps1
