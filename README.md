...
**Текущий статус:** Production-ready • 165 тестов • Sprint 7 завершён ✅ • AI Integration планируется (Sprint 8) 🔜
...
### UI
- `GET /ui` - HTML витрина (однофайловый UI)
...
В проекте есть утилита для удаления тестовых/интеграционных артефактов из Postgres: `scripts/cleanup_test_data.py`.

Примеры:

```powershell
# dry-run (по умолчанию) — покажет, что будет удалено
python scripts/cleanup_test_data.py

# удалить все коды по префиксу (например, интеграционные тесты)
...
Скрипт:
- читает переменные из `.env` (при наличии) и/или `PG*`/`DB_*`,
- сначала удаляет записи в дочерних таблицах по FK, затем из `public.products`,
